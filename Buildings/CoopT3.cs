using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Tilemaps;

public class CoopT3 : Coop {

    // public CoopT3(Vector3Int[] position, Vector3Int[] basePosition, Tilemap tilemap) : base(position, basePosition, tilemap) {
    //     Init();
    // }

    // public CoopT3() : base() {
    //     Init();
    // }

    protected override void Init(){
        name = GetType().Name;
        texture = Resources.Load("Buildings/Deluxe Coop") as Texture2D;
        insideAreaTexture = Resources.Load("BuildingInsides/Coop3") as Texture2D;
        _materialsNeeded = new Dictionary<Materials, int> {
            {Materials.Coins, 4000 + 10000 + 20000},
            {Materials.Wood, 300 + 400 + 500},
            {Materials.Stone, 100 + 150 + 200}
        };
        base.Init();
    }
}
